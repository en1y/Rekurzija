...
def a(needed_sum):
    ...
    def b(identificator, new_sum):
        ...
        If the new_sum subtracted by (identificator + 1) is greater than 0 it will return itself with the use of the (identificator + 1) and without it, so we can look at all the possibilities
        If the new_sum subtracted by (identificator + 1) is smaller than 0 it will return itself with decreased identificator"""
        if identificator == -1 and new_sum == 0:
            return 1
        if identificator == -1 and new_sum != 0:
            return 0
        if new_sum == (identificator + 1):
            if identificator >= 1:
                return 1 + b(identificator - 1, new_sum)
            else:
                return 1
        if new_sum - (identificator + 1) > 0:
            return b(identificator-1, new_sum - (identificator + 1)) + b(identificator-1, new_sum)
        else:
            return b(identificator-1, new_sum)
    return b(needed_sum-2, needed_sum)
